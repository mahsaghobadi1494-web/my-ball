...
Arena.prototype.build = function (R) {
  ...
  // =========================================================================
  // OUTSIDE SPECTATOR GRANDSTANDS (Spaciously separated from pitch & walls)
  // =========================================================================
  var bGrandstands = new Builder();
  var gTiers = [
    // 1. Wide Outer Perimeter Concourse / Plaza surrounding the cage (Ground level)
    { d0: 0.0, y0: 0.0, d1: 14.0, y1: 0.0, v0: 0.00, v1: 0.08 },
    // 2. Imposing Stadium Perimeter Retaining Concrete Wall (Height 0 to 5.2m)
    { d0: 14.0, y0: 0.0, d1: 14.0, y1: 5.2, v0: 0.08, v1: 0.14 },
    // 3. Lower Concourse Promenade & Railing Walkway
    { d0: 14.0, y0: 5.2, d1: 17.0, y1: 5.2, v0: 0.14, v1: 0.18 },
    // 4. Tier 1 - Lower Seating Bowl (Steep stadium raking from y=5.2 to y=15.8)
    { d0: 17.0, y0: 5.2, d1: 31.0, y1: 15.8, v0: 0.18, v1: 0.45 },
    // 5. Middle Concourse Walkway
    { d0: 31.0, y0: 15.8, d1: 34.0, y1: 15.8, v0: 0.45, v1: 0.49 },
    // 6. Tier 2 - Middle Seating Bowl
    { d0: 34.0, y0: 17.0, d1: 49.0, y1: 28.2, v0: 0.49, v1: 0.74 },
    // 7. Upper Balcony Concourse
    { d0: 49.0, y0: 28.2, d1: 52.0, y1: 28.2, v0: 0.74, v1: 0.78 },
    // 8. Tier 3 - Upper Deck
    { d0: 52.0, y0: 29.5, d1: 67.0, y1: 41.0, v0: 0.78, v1: 1.00 },
    // 9. Majestic Stadium Canopy Overhang
    { d0: 67.0, y0: 41.5, d1: 36.0, y1: 47.0, v0: 0.95, v1: 0.98 }
  ];
  ...
  // Perimeter Ad Boards (LED Sponsor Panels)
  var bAdBoards = new Builder();
  var adH0 = 0.15, adH1 = 1.60;
  for (var s0 = 0; s0 < nPts; s0++) {
    var s1 = (s0 + 1) % nPts;
    var A0 = rings[0][s0], A1 = rings[0][s1];
    var u0 = perim[s0] * 0.15, u1 = (s1 === 0 ? perim[nPts - 1] + 2 : perim[s1]) * 0.15;
    var i0 = bAdBoards.vert(A0.x, adH0, A0.z, -A0.nx, 0, -A0.nz, u0, 0);
    var i1 = bAdBoards.vert(A1.x, adH0, A1.z, -A1.nx, 0, -A1.nz, u1, 0);
    var i2 = bAdBoards.vert(A1.x, adH1, A1.z, -A1.nx, 0, -A1.nz, u1, 1);
    var i3 = bAdBoards.vert(A0.x, adH1, A0.z, -A0.nx, 0, -A0.nz, u0, 1);
    bAdBoards.quadN(i0, i1, i2, i3);
  }

  // Upper Tier LED Ribbons
  var bRibbons = new Builder();
  var ribH0 = h * 0.52, ribH1 = h * 0.60;
  var ribRing = rings[Math.floor(rings.length * 0.55)] || rings[0];
  for (var s0 = 0; s0 < nPts; s0++) {
    var s1 = (s0 + 1) % nPts;
    var A0 = ribRing[s0], A1 = ribRing[s1];
    var u0 = perim[s0] * 0.08, u1 = (s1 === 0 ? perim[nPts - 1] + 2 : perim[s1]) * 0.08;
    var i0 = bRibbons.vert(A0.x, ribH0, A0.z, -A0.nx, 0, -A0.nz, u0, 0);
    var i1 = bRibbons.vert(A1.x, ribH0, A1.z, -A1.nx, 0, -A1.nz, u1, 0);
    var i2 = bRibbons.vert(A1.x, ribH1, A1.z, -A1.nx, 0, -A1.nz, u1, 1);
    var i3 = bRibbons.vert(A0.x, ribH1, A0.z, -A0.nx, 0, -A0.nz, u0, 1);
    bRibbons.quadN(i0, i1, i2, i3);
  }
