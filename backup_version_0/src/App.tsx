...
import { CFG, DEFAULT_CFG, TEAM, TEAM_NAME, TEAM_COLOR } from "./game/config.js";
...
export default function App() {
  ...
  return (
    <main id="app" className="relative w-full h-full overflow-hidden select-none font-sans text-neutral-100">
      ...
      {/* ============================================================ */}
      {gameState === "MENU" && (
        <div id="menu" className="screen z-20 flex flex-col justify-between p-6 sm:p-10 lg:p-16 overflow-y-auto">
          {/* Header */}
          <header className="flex items-start justify-between gap-6 flex-wrap">
            <div>
              <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black italic tracking-tighter uppercase text-white leading-none">
                OVERDRIVE <span className="text-[#ff3385]">STRIKERS</span>
              </h1>
              <p className="mt-2 text-sm sm:text-base text-neutral-300 max-w-xl font-light">
            ...
            <div className="text-right font-mono text-xs text-neutral-400 flex flex-col items-end gap-1">
              <span className="px-2.5 py-1 bg-neutral-800/80 border border-white/10 rounded text-[#99fa47] font-semibold">
                ENGINE V2.4 • 240HZ PHY
              </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 my-8 items-center">
            {/* Play Modes List */}
            <div className="lg:col-span-2 flex flex-col border-t border-white/10">
              {[
                { id: "2v2", label: "2v2 Standard Match", meta: "Competitive Doubles Match • 5 Min" },
                { id: "3v3", label: "3v3 Stadium Brawl", meta: "Full Arena Chaos • 6 Vehicles" },
                { id: "1v1", label: "1v1 Duel", meta: "Pure Mechanical Skill Showdown" },
                { id: "freeplay", label: "Freeplay Practice", meta: "Solo Arena • Infinite Boost & Ball Training" }
              ].map((item, idx) => (
                <button
                  key={item.id}
                  onClick={() => handleStartGame(item.id)}
                  className="group flex items-center justify-between py-4 px-3 border-b border-white/10 hover:bg-white/5 hover:pl-6 transition-all text-left"
                >
                  <div className="flex items-center gap-4">
                    <span className="font-mono text-xs text-neutral-500 group-hover:text-[#ff3385]">0{idx + 1}</span>
                    <div>
                      <h3 className="text-xl sm:text-2xl font-bold tracking-tight text-white group-hover:text-[#99fa47] transition">
                        {item.label}
                      </h3>
                      <p className="text-xs font-mono text-neutral-400 mt-0.5">{item.meta}</p>
                    </div>
                  </div>
                  <ChevronRight className="w-5 h-5 text-neutral-500 group-hover:text-white transition group-hover:translate-x-1" />
                </button>
              ))}
              ...
              <div className="flex gap-3 mt-6 flex-wrap">
                <button
                  onClick={() => setShowTuningPanel(prev => !prev)}
                  className="btn flex items-center gap-2 px-4 py-2.5 rounded bg-[#99fa47]/20 border border-[#99fa47]/40 hover:bg-[#99fa47]/30 text-xs font-mono uppercase tracking-wider text-[#99fa47] shadow-[0_0_12px_rgba(153,250,71,0.2)]"
                >
                  <Sliders className="w-4 h-4" />
                  <span>Real-Time Tuning [T]</span>
                </button>
                <button
                  onClick={() => setActivePanel("settings")}
                  className="btn flex items-center gap-2 px-4 py-2.5 rounded bg-neutral-800/80 hover:bg-neutral-700 border border-white/10 text-xs font-mono uppercase tracking-wider"
                >
            ...
            {/* Team & Bot Selection Sidebar */}
            <div className="bg-neutral-900/80 backdrop-blur-md p-6 rounded-xl border border-white/10 flex flex-col gap-6">
              <div>
                <span className="text-[11px] font-mono uppercase tracking-widest text-[#99fa47] font-semibold">Choose Your Team</span>
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <button
                    onClick={() => setPlayerTeam(TEAM.PULSE)}
                    className={`py-3 px-4 rounded-lg font-bold text-sm flex items-center justify-between border transition ${playerTeam === TEAM.PULSE ? 'bg-[#ff3385]/20 border-[#ff3385] text-[#ff3385]' : 'bg-neutral-800/50 border-white/5 text-neutral-400'}`}
                  >
                    <span>PULSE</span>
                  ...
                  <button
                    ...
                    className={`py-3 px-4 rounded-lg font-bold text-sm flex items-center justify-between border transition ${playerTeam === TEAM.VOLT ? 'bg-[#99fa47]/20 border-[#99fa47] text-[#99fa47]' : 'bg-neutral-800/50 border-white/5 text-neutral-400'}`}
                  >
                    <span>VOLT</span>
              ...
              <div>
                <span className="text-[11px] font-mono uppercase tracking-widest text-[#99fa47] font-semibold">AI Bot Difficulty</span>
                <div className="grid grid-cols-3 gap-2 mt-3">
                  {[
                    ...
                    <button
                      ...
                      className={`py-2 px-2 rounded font-mono text-xs font-semibold border transition ${botSkill === b.level ? 'bg-white/15 border-white text-white' : 'bg-neutral-800/40 border-white/5 text-neutral-400'}`}
                    >
                      {b.label}
                    </button>
                  ))}
              ...
              <div className="p-3.5 bg-black/40 rounded-lg border border-white/5 text-xs text-neutral-300 font-mono leading-relaxed">
                <span className="text-[#ff3385] font-bold">Quick Controls:</span>
                <p className="mt-1">WASD / Arrows to Drive & Pitch</p>
                <p>SPACE to Jump & Double-Jump / Dodge</p>
                <p>SHIFT / RMB to Boost • Q / E to Air Roll</p>
                <p>C to Toggle Ballcam</p>
              </div>
            </div>
          </div>

          {/* Footer */}
          <footer className="flex justify-between items-center border-t border-white/10 pt-4 text-xs font-mono text-neutral-500 flex-wrap gap-2">
            <span>READY TO STRIKE • GAMEPAD SUPPORTED</span>
            {lastMatchResult && (
