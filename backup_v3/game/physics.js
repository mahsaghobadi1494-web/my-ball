...
Arena.prototype.build = function (R) {
  ...
  // VIP LUXURY SKYBOX SUITES (Glass facade running around upper concourse balcony y = 27.2)
  var bVIPSuites = new Builder();
  var vipD0 = 54.2, vipD1 = 56.2, vipY0 = 27.2, vipY1 = 28.5;
  for (var vsi = 0; vsi < nPts; vsi++) {
    var vsiNext = (vsi + 1) % nPts;
    var vA0 = rings[0][vsi], vA1 = rings[0][vsiNext];
    var vo0x = -vA0.nx, vo0z = -vA0.nz;
    var vo1x = -vA1.nx, vo1z = -vA1.nz;

    var vp0x = vA0.x + vo0x * vipD0, vp0z = vA0.z + vo0z * vipD0;
    var vp1x = vA1.x + vo1x * vipD0, vp1z = vA1.z + vo1z * vipD0;
    var vp2x = vA1.x + vo1x * vipD1, vp2z = vA1.z + vo1z * vipD1;
    var vp3x = vA0.x + vo0x * vipD1, vp3z = vA0.z + vo0z * vipD1;

    // Glass panel window front
    var vvi0 = bVIPSuites.vert(vp0x, vipY0, vp0z, -vo0x, 0, -vo0z, 0, 0);
    var vvi1 = bVIPSuites.vert(vp1x, vipY0, vp1z, -vo1x, 0, -vo1z, 1, 0);
    var vvi2 = bVIPSuites.vert(vp1x, vipY1, vp1z, -vo1x, 0, -vo1z, 1, 1);
    var vvi3 = bVIPSuites.vert(vp0x, vipY1, vp0z, -vo0x, 0, -vo0z, 0, 1);
    bVIPSuites.quadN(vvi0, vvi1, vvi2, vvi3);

    // Structural Chrome Divider Mullions every 2 segments
    if (vsi % 2 === 0) {
      bVIPSuites.box(0.2, (vipY1 - vipY0) * 0.5, 0.2, new V3(vp0x, (vipY0 + vipY1) * 0.5, vp0z), null, 0.5);
    }
  }
  ...
  var bRoofCanopy = new Builder();
  var bRoofSuperArch = new Builder();
  var bRoofSpotlights = new Builder();

  var roofPeakY = h + 4.5;
  var roofBaseY = h + 0.2;

  // 1. Translucent Canopy Membrane Shell (Curving gracefully over grandstand seating bowl)
  var canopyOuterD = 71.0, canopyInnerD = 36.0;
  var canopyOuterY = 39.5, canopyInnerY = 44.5;
  for (var csi = 0; csi < nPts; csi++) {
    var csiNext = (csi + 1) % nPts;
    var cA0 = rings[0][csi], cA1 = rings[0][csiNext];
    var co0x = -cA0.nx, co0z = -cA0.nz;
    var co1x = -cA1.nx, co1z = -cA1.nz;

    var cp0x = cA0.x + co0x * canopyOuterD, cp0z = cA0.z + co0z * canopyOuterD;
    var cp1x = cA1.x + co1x * canopyOuterD, cp1z = cA1.z + co1z * canopyOuterD;
    var cp2x = cA1.x + co1x * canopyInnerD, cp2z = cA1.z + co1z * canopyInnerD;
    var cp3x = cA0.x + co0x * canopyInnerD, cp3z = cA0.z + co0z * canopyInnerD;

    // Canopy Membrane Quad Panels
    var cvi0 = bRoofCanopy.vert(cp0x, canopyOuterY, cp0z, 0, -1, 0, 0, 0);
    var cvi1 = bRoofCanopy.vert(cp1x, canopyOuterY, cp1z, 0, -1, 0, 1, 0);
    var cvi2 = bRoofCanopy.vert(cp2x, canopyInnerY, cp2z, 0, -1, 0, 1, 1);
    var cvi3 = bRoofCanopy.vert(cp3x, canopyInnerY, cp3z, 0, -1, 0, 0, 1);
    bRoofCanopy.quadN(cvi0, cvi1, cvi2, cvi3);

    // Inner Canopy Overhang Edge Light Strips & Spotlights
    bRoofSpotlights.box(1.2, 0.35, 1.2, new V3(cp3x, canopyInnerY - 0.2, cp3z), null, 1.0);
    if (csi % 3 === 0) {
      bRoofSpotlights.box(0.6, 0.6, 0.6, new V3(cp3x, canopyInnerY - 0.5, cp3z), null, 1.0);
    }
  }

  // 2. Twin Monumental Steel Super-Arches (Towering 60m above field with suspension cable harp network)
  var archXOffset = this.hx * 0.72;
  var archZSpan = this.hz * 1.35;
  var archMaxY = h + 28.0; // Towering high above arena ceiling
  var archSegs = 32;

  for (var side = -1; side <= 1; side += 2) {
    var archX = side * archXOffset;
    for (var asi = 0; asi < archSegs; asi++) {
      var at0 = (asi / archSegs) * 2.0 - 1.0;
      var at1 = ((asi + 1) / archSegs) * 2.0 - 1.0;

      var az0 = at0 * archZSpan;
      var az1 = at1 * archZSpan;
      var ay0 = roofBaseY + (1.0 - at0 * at0) * (archMaxY - roofBaseY);
      var ay1 = roofBaseY + (1.0 - at1 * at1) * (archMaxY - roofBaseY);

      // Heavy Tubular Main Arch Spine
      bRoofSuperArch.tube(new V3(archX, ay0, az0), new V3(archX, ay1, az1), 0.55, 10);
      bRoofSuperArch.tube(new V3(archX * 1.04, ay0 - 0.8, az0), new V3(archX * 1.04, ay1 - 0.8, az1), 0.35, 8);
      // Diagonal internal arch trussing
      bRoofSuperArch.tube(new V3(archX, ay0, az0), new V3(archX * 1.04, ay1 - 0.8, az1), 0.15, 6);

      // Vertical & Diagonal Steel Suspension Cable Harp Array going down to canopy roof
      if (asi % 2 === 0 && Math.abs(az0) < archZSpan * 0.88) {
        var cableTargetY = canopyInnerY + 1.2;
        var cableTargetX = side * (this.hx * 0.60);
        bRoofSuperArch.tube(new V3(archX, ay0 - 0.8, az0), new V3(cableTargetX, cableTargetY, az0), 0.08, 6);
      }
    }
  }

  // Cross-trusses linking the twin super-arches at peak
  for (var cz = -this.hz * 0.8; cz <= this.hz * 0.8; cz += 24.0) {
    var cRatio = cz / archZSpan;
    var cY = roofBaseY + (1.0 - cRatio * cRatio) * (archMaxY - roofBaseY);
    bRoofSuperArch.tube(new V3(-archXOffset, cY, cz), new V3(archXOffset, cY, cz), 0.35, 8);
    bRoofSuperArch.tube(new V3(-archXOffset, cY, cz), new V3(archXOffset, cY, cz + 12.0), 0.15, 6);
  }

  // 3. Longitudinal Arched Spine Girders (Center keel + 2 lateral arched spines)
  ...
  // 4. Transverse Cantilever Arch Ribs (Curving across the stadium width every 16m)
  ...
  // 5. Giant Suspended Celestial Energy Halo Ring above Center Field
  var haloRadius = 15.5;
  ...
  return {
    ...
    roofCanopy: R.mesh(bRoofCanopy), roofSuperArch: R.mesh(bRoofSuperArch),
    vipSuites: R.mesh(bVIPSuites), roofSpotlights: R.mesh(bRoofSpotlights),
    beams: R.mesh(bBeams)
  };
};
