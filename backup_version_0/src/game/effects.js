...
Effects.prototype.tyreDust = function (car, wheel, amount, dt) {
  if (this.rng.next() > amount * 22 * dt * CFG.gfx.particles) return;
  var p = this.spawn();
  if (!p) return;
  p.pos.copy(wheel.contact);
  p.pos.y += 0.04;
  p.vel.set(car.body.vel.x * 0.18 + this.rng.range(-0.7, 0.7), this.rng.range(0.3, 1.1), car.body.vel.z * 0.18 + this.rng.range(-0.7, 0.7));
  p.col[0] = 0.55; p.col[1] = 0.55; p.col[2] = 0.62;
  p.size = this.rng.range(0.14, 0.3);
  p.max = this.rng.range(0.3, 0.6);
  p.life = 0; p.grav = -0.4; p.drag = 2.2; p.fade = 0.42; p.grow = 0.7; p.stretch = 1;
};
