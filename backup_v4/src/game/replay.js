...
ReplayManager.prototype.start = function (seconds, speed, goalTime) {
  ...
  this.playT = 0;
  this.speed = speed || 1;
  this.goalTime = goalTime || 0;
  this.playing = true;
  return true;
};
ReplayManager.prototype.startGoalReplay = function (preGoalSecs, postGoalSecs, goalHead, goalTime, speed) {
  if (this.filled < 8) return false;
  var preFrames = Math.ceil((preGoalSecs || 9.0) * this.hz);
  var postFrames = Math.ceil((postGoalSecs || 1.2) * this.hz);

  var anchorHead = (goalHead !== undefined && goalHead >= 0) ? goalHead : this.head;
  var actualPre = Math.min(preFrames, this.filled);
  var totalSpan = actualPre + postFrames;

  this.playSpan = totalSpan;
  this.playFrom = (anchorHead - actualPre + this.frames) % this.frames;
  this.playT = 0;
...
ReplayManager.prototype.sample = function (dt, cars, ball) {
  ...
  var currentI = Math.floor(this.playT);
  var currentA = (this.playFrom + currentI) % this.frames;
  var sampleTime = this.buf[currentA * this.stride];

  // Dynamic slow motion as the ball enters the goal (~1.8s before goal impact/crossing)
  var currentSpeed = this.speed;
  if (this.goalTime && sampleTime > 0) {
    var tDiff = this.goalTime - sampleTime; // time until goal event
    if (tDiff <= 1.8 && tDiff >= -0.5) {
      var slowBlend = 0;
      if (tDiff > 0.4) {
        slowBlend = (1.8 - tDiff) / 1.4; // Smoothly ramp into slow motion
      } else if (tDiff >= -0.2) {
        slowBlend = 1.0; // Peak slow motion right as ball crosses goal line!
      } else {
        slowBlend = (tDiff - (-0.5)) / 0.3; // Ease out back to normal
      }
      slowBlend = Math.max(0, Math.min(1, slowBlend));
      slowBlend = slowBlend * slowBlend * (3.0 - 2.0 * slowBlend); // Smooth curve
      currentSpeed = lerp(this.speed, this.speed * 0.35, slowBlend);
    }
  }

  this.playT += dt * currentSpeed * this.hz;
