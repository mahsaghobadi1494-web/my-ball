...
export function CarCustomizerDrawer({
  ...
  const [activeTab, setActiveTab] = useState("models"); // 'models' | 'wheels' | 'cosmetics' | 'vinyls' | 'celebrations' | 'paint' | 'presets'
  const [vinylCategory, setVinylCategory] = useState("ALL");
  const [cosmeticCategory, setCosmeticCategory] = useState("ALL");
  ...
  const [cust, setCust] = useState(() => {
    ...
    return {
      ...
      horn: current.horn || "none",
      tool: current.tool || "none",
      wing: current.wing || "none",
      celebration: current.celebration || "MASSIVE_EXPLOSION",
  ...
  const updateParam = (key, value) => {
    ...
    if (key === "model" || key === "wheel" || key === "hat" || key === "horn" || key === "tool" || key === "wing" || key === "celebration") {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3.5 bg-gradient-to-r from-neutral-900/90 via-neutral-900/60 to-neutral-950/90 border-b border-white/10">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-neutral-400">۲۰ مدل بدنه، ۴۰ رینگ، ۵۵ تزیینات، ۳۲ وینیل و ۲۰ افکت جشن گل</p>
          </div>
        </div>
      ...
      {/* 6 Navigation Tabs */}
      <div className="grid grid-cols-6 border-b border-white/10 bg-neutral-900/50 p-1 gap-1">
        <button
          onClick={() => setActiveTab("models")}
          className={`flex flex-col items-center justify-center py-1.5 px-0.5 rounded-lg text-[10px] font-medium transition ${
            activeTab === "models"
        ...
        >
          <Disc className="w-3.5 h-3.5 text-cyan-400 mb-0.5" />
          <span>رینگ</span>
        </button>
        ...
        >
          <Flame className="w-3.5 h-3.5 text-red-400 mb-0.5" />
          <span>جشن گل</span>
        </button>
        ...
        >
          <Palette className="w-3.5 h-3.5 text-emerald-400 mb-0.5" />
          <span>رنگ/تیم</span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar">
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-neutral-200">
                  انتخاب مدل رینگ و لاستیک (۴۰ مدل جدید)
                </h3>
              </div>
        ...
        {/* ========================================================= */}
        {/* TAB 3: COSMETICS & TOPPERS (55 ITEMS) */}
        {/* ========================================================= */}
        {activeTab === "cosmetics" && (
          <div className="space-y-4 animate-in fade-in duration-200">
            <div className="p-3 rounded-xl bg-gradient-to-r from-purple-500/10 via-pink-500/10 to-blue-500/10 border border-purple-500/20">
              <div className="text-xs font-bold text-purple-300 flex items-center gap-1.5 mb-1">
                <Shield className="w-3.5 h-3.5" />
                <span>کلاه‌ها، شاخ‌ها، ابزارها و بال‌های تزئینی (۵۵ آیتم کامل)</span>
              </div>
              <p className="text-[11px] text-neutral-300 leading-relaxed">
                آیتم‌های سقف و بدنه خودرو شامل کلاه‌های کلاسیک، شاخ‌های اژدها، ابزارهای رزمی و بال‌های پرواز.
              </p>
            </div>
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 custom-scrollbar text-[11px] font-medium">
              {[
                ...
                { id: "horns", label: "🐂 شاخ‌ها و تیغه‌ها (۱۵)" },
                { id: "hats", label: "🎩 کلاه‌ها و تاپرها (۱۵)" },
                { id: "tools", label: "⚔️ ابزارها و سلاح‌ها (۱۵)" },
                { id: "wings", label: "🦅 بال‌ها و بوسترها (۱۰)" }
              ].map((cat) => {
                const isCatActive = cosmeticCategory === cat.id;
                return (
                  <button
                    key={cat.id}
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {COSMETICS_LIBRARY.filter(item => cosmeticCategory === "ALL" || item.category === cosmeticCategory).map((item) => {
                const isSelected = cust[item.category === 'horns' ? 'horn' : item.category === 'hats' ? 'hat' : item.category === 'tools' ? 'tool' : 'wing'] === item.id;
                return (
                  <button
                    key={item.id}
                    onClick={() => {
                      const paramKey = item.category === 'horns' ? 'horn' : item.category === 'hats' ? 'hat' : item.category === 'tools' ? 'tool' : 'wing';
                      updateParam(paramKey, isSelected ? 'none' : item.id);
                    }}
                    className={`relative text-right p-3 rounded-xl border transition-all duration-200 flex flex-col justify-between ${
                      isSelected
                        ? "bg-purple-950/30 border-purple-500/80 shadow-[0_0_16px_rgba(168,85,247,0.25)] ring-1 ring-purple-500/50"
                        : "bg-neutral-900/60 border-white/10 hover:border-white/20 hover:bg-neutral-800/60"
                    }`}
                  >
                    {isSelected && (
                      <div className="absolute top-2 left-2 w-4 h-4 rounded-full bg-purple-400 text-neutral-950 flex items-center justify-center">
                        <Check className="w-2.5 h-2.5 stroke-[3]" />
                      </div>
                    )}
        ...
        {/* ========================================================= */}
        {/* TAB 4: CELEBRATION EFFECTS (20 GOAL EFFECTS) */}
        {/* ========================================================= */}
        {activeTab === "celebrations" && (
          <div className="space-y-4 animate-in fade-in duration-200">
            <div className="p-3 rounded-xl bg-gradient-to-r from-red-500/10 via-amber-500/10 to-purple-500/10 border border-red-500/20">
              <div className="text-xs font-bold text-red-300 flex items-center gap-1.5 mb-1">
                <Flame className="w-3.5 h-3.5" />
                <span>افکت‌های ویژه جشن پس از گل (۲۰ افکت انیمیشنی)</span>
              </div>
              <p className="text-[11px] text-neutral-300 leading-relaxed">
                افکت‌های انفجار غول‌پیکر، آتش‌بازی، گرداب سیاهچاله و بارش طلا هنگام گل زدن.
              </p>
            </div>

            <div className="grid grid-cols-2 gap-2.5">
              {CELEBRATION_EFFECTS.map((eff) => {
                const isSelected = cust.celebration === eff.id;
                return (
                  <button
                    key={eff.id}
                    onClick={() => updateParam("celebration", eff.id)}
                    className={`relative text-right p-3 rounded-xl border transition-all duration-200 flex flex-col justify-between ${
                      isSelected
                        ? "bg-red-950/30 border-red-500/80 shadow-[0_0_16px_rgba(239,68,68,0.25)] ring-1 ring-red-500/50"
                        : "bg-neutral-900/60 border-white/10 hover:border-white/20 hover:bg-neutral-800/60"
                    }`}
                  >
                    {isSelected && (
                      <div className="absolute top-2 left-2 w-4 h-4 rounded-full bg-red-400 text-neutral-950 flex items-center justify-center">
                        <Check className="w-2.5 h-2.5 stroke-[3]" />
                      </div>
                    )}
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: eff.color }} />
                        <span className="text-xs font-bold text-white">{eff.nameFa}</span>
                      </div>
                      <div className="text-[10px] text-red-300/80 font-mono mt-0.5">
                        {eff.name}
                      </div>
                    </div>
